...
//inline bool operator== (const Vertex& lfs, const Vertex& rfs)
//{
//	float delta = 1e-5;
//	if(fabs(lfs.x - rfs.x) < delta && fabs(lfs.y - rfs.y) < delta 
//		&& fabs(lfs.z - rfs.z) < delta)
//		return true;
//
//	return false;
//}
...
struct Color {
	...
	inline Color& operator+= (const Color &src);
	inline Color& operator*= (const Color &src);
	inline Color& operator/= (const Color &src);
	inline Color& operator/= (float ratio);
...
inline Color& Color::operator/= (float ratio)
{
	r /= ratio;
	g /= ratio;
	b /= ratio;
...
struct Triangle {
	...
	int vertexIndex[3]; ///< Triangle's global vertex index
