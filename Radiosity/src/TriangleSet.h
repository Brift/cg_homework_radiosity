...
public:
     ...
     */
    void add(const TriangleSet &tset);
	...
	const Triangle& at(unsigned index) const
	{
		return container_[index];
	}

	Triangle& at(unsigned index)
	{
		return container_[index];
	}
    /**
     ...
     */
    template <int QUALITY> static float size (const Triangle &triangle);
     ...
     */
	static boost::shared_ptr<TriangleSet> divide(const Triangle &triangle, int &initializeCount);

	static boost::shared_ptr<TriangleSet> divide(const TriangleSet &tSet, int &initializeCount);

	static boost::shared_ptr<TriangleSet> divideBySize(const TriangleSet &tSet, float size);

	static void recuriveDivideBySize(TriangleSet &afterDivide, const Triangle &t, float size, 
		int &initializeCount);

	int getTotalVertexCount() const { return vertexCount; }
	void setTotalVertexCount(int count) { vertexCount = count; }

private:
    typedef std::vector<Triangle> TContainer;
    TContainer container_;
	int vertexCount;	///< global vertex count 
};
...
/**
	...
	*/
template <> inline float TriangleSet::size<0> (const Triangle &t)
{
    const Vertex *v = t.vertex;
    float size = 0.0;
    for(int i=0; i<3; i++)
...
/**
	...
	*/
template <> inline float TriangleSet::size<1> (const Triangle &t)
{
    const Vertex *v = t.vertex;
    float length[3];
    float sum = 0.0;
