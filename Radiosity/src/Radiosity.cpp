...
int main(int argc, char **argv)
{
	...
	RadiosityParserArguments parser;
	parser.setProDescription(description);
	try{
		parser.parseArguments(argc, argv);
	}catch(const ArgumentsParseFailException &e){
	...
	objLoader objData;
	double dTriangleArea = 0.0;
	bool bTakeScreenShot = false;
	try{
		if(!objData.load(parser.getFileName().c_str()))
		{
			std::cerr << "Invalide Obj file Name" << std::endl;
			exit(-1);
		}
		
		dTriangleArea  = parser.getTriArea();
		bTakeScreenShot = parser.getScreenShot();
	}catch(const ArgumentsParseFailException &e){
		std::cerr << e.what() << std::endl;
		parser.showHelp();
		exit(-1);
	...
	RadiosityEngine engine(&objData, dTriangleArea);
	engine.computeNSteps(32);
	
	Visualizer::visualize("cornell_box", engine, bTakeScreenShot);
}